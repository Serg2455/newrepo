...
class CraftTask < Task
    ...
    # @param [Tile] tile - The tile to craft
    # @param [Boolean] override_can_perform - (default: false)
    # @return [void]
    def initialize args, assignee, tile, override_can_perform=false, craft_count=3
        @goal = {
        ...
        tile.desired_craft_count = craft_count 
        tile.crafted_item_count = 0

        super('Craft', args, assignee, override_can_perform)
    end
    ...
    def tick args
        ...
        # If the goal is no longer a craftable tile or is already crafting
        ...
        # If the tile does not have enough resources to begin crafting
        ...
        if @goal.nil?
            # begins the crafting
            tile.hit_points += @assignee.crafting_mult
            tile.hit_points = [ tile.hit_points, tile.total_hit_points ].min
            @assignee.crafting_experience_points += 0.1
            return
        end
    ...
    # NOTE: True by default. Some tasks, like the craft task, may not be
    ...
    def performable args, performee
        ...
        if not tile.has_required_resources?
            ...
            # pick up task and eventually perform the craft task
            return true
        end
    ...
    def to_s
        return 'Crafting wooden swords' #+ @target_name
    end
end
